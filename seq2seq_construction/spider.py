...
def spider_add_serialized_schema(ex: dict, args) -> dict:
    if getattr(args.seq2seq, "schema_serialization_with_nl"):
        ...
        serialized_schema = serialize_schema(
            ...
            schema_serialization_type="Patrick",
            schema_serialization_randomized=False,
            schema_serialization_with_db_id=True,
            schema_serialization_with_db_content=args.seq2seq.schema_serialization_with_db_content,
            normalize_query=True,
...
def serialize_schema(
    ...
    elif schema_serialization_type == "Patrick":
        db_id_str = " | database_name: {db_id}"
        table_sep = ""
        table_str = " | Table {table} {columns}"
        column_sep = " "
        column_str_with_values = "{Type} {column} ( {values} )"
        column_str_without_values = "{Type} {column}"
        value_sep = " , "

    else:
        raise NotImplementedError

    # construct a dict mapping id of db_columns -> db_types:
    column_types_mapping = defaultdict(str)
    for i, ty in enumerate(db_column_types):
        column_types_mapping[i] = ty
    ...
    #     else:
    #         return column_str_without_values.format(column=column_name_str)
    def get_column_str(table_name: str, column_name: str, column_type: str) -> str:
        column_name_str = column_name.lower() if normalize_query else column_name
        ...
        if schema_serialization_with_db_content:
            ...
            if matches:
                return column_str_with_values.format(
                    Type=column_type, column=column_name_str, values=value_sep.join(matches)
                )
            else:
                return column_str_without_values.format(Type = column_type, column=column_name_str)
        else:
            return column_str_without_values.format(Type = column_type, column=column_name_str)
    ...
    tables = [
        table_str.format(
            ...
            columns=column_sep.join(
                map(
                    lambda y: get_column_str(table_name=table_name, column_name=y[1], column_type=y[2]),
                    filter(
